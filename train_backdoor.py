...
from data.data_loader import load_dataset_h5

import models
import data.poison_cifar as poison
from data.data_loader import CustomCifarBDDataSet
...
parser.add_argument('--batch_size', type=int, default=64, help='the batch size for dataloader')
...
parser.add_argument('--poison_type', type=str, default='badnets', choices=['badnets', 'blend', 'clean-label', 'benign'],
...
def main():
    ...
    orig_train = CustomCifarBDDataSet(args.data_set, train=True, transform=transform_train)
    ...
    clean_test = CustomCifarBDDataSet(args.data_set, train=False, transform=transform_test)
    ...
    torch.save(net.state_dict(), os.path.join(args.output_dir, 'model_' + str(args.poison_type) + '_init.th'))
    ...
    torch.save(net.state_dict(), os.path.join(args.output_dir, 'model_' + str(args.poison_type) + '_last.th'))
